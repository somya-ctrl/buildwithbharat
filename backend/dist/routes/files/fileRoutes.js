...
router.get('/workspace/:id', authMiddleware_1.protect, async (req, res) => {
...
router.post('/', authMiddleware_1.protect, async (req, res) => {
    ...
    try {
        ...
        if (parentId) {
            const parent = await prisma_1.default.file.findUnique({
                where: { id: parentId },
            });
        ...
        const file = await prisma_1.default.$transaction(async (tx) => {
...
router.patch('/:id', authMiddleware_1.protect, async (req, res) => {
    ...
    try {
        ...
        const updated = await prisma_1.default.file.update({
            where: { id },
            data: { name },
        });
        res.json(updated);
...
router.delete('/:id', authMiddleware_1.protect, async (req, res) => {
    ...
    try {
        ...
        await prisma_1.default.file.delete({
            where: { id },
        });
...
router.get('/:id/content', authMiddleware_1.protect, async (req, res) => {
    const id = req.params.id;
    const userId = req.user?.id;
    try {
        const file = await checkFileAccess(id, userId);
...
router.put('/:id/content', authMiddleware_1.protect, async (req, res) => {
    const id = req.params.id;
    ...
    try {
        ...
        const updatedFile = await prisma_1.default.$transaction(async (tx) => {
